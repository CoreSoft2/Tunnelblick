...
#
# The 'restore_ipv6' routine in client.down.halonet.sh undoes the actions performed by this routine.
#
...
# special-case fiddling:
	...
	scutil <<-EOF > /dev/null
		...
		# If there isn't a key, "HalonetNoSuchKey : true" won't be removed.
		# If there is a key, "HalonetNoSuchKey : true" will be removed and the key's contents will be used
...
#               set additional search domains. (Halonet-specific addition.)
#
		...
		if [ "${log_dns_info_new_dns_sa}" == "127.0.0.1" ] ; then
			...
			if [ "${serversContainLoopback}" != "" ] ; then
				...
				if ${knownDnsServerNotFound} ; then
					logMessage "NOTE: The DNS servers do not include any free public DNS servers known to Halonet. This may cause DNS queries to fail or be intercepted or falsified even if they are directed through the VPN. Specify only known public DNS servers or DNS servers located on the VPN network to avoid such problems."
				else
					if ${unknownDnsServerFound} ; then
						logMessage "NOTE: The DNS servers include one or more free public DNS servers known to Halonet and one or more DNS servers not known to Halonet. If used, the DNS servers not known to Halonet may cause DNS queries to fail or be intercepted or falsified even if they are directed through the VPN. Specify only known public DNS servers or DNS servers located on the VPN network to avoid such problems."
					else
						logMessage "The DNS servers include only free public DNS servers known to Halonet."
					fi
				fi
			fi
		fi
...
readonly TBALTPREFIX="/Library/Application Support/Halonet/Users/"
...
if [ "$TBCONFIGSTART" = "$TBALTPREFIX" ] ; then
	...
	readonly TBCONFIG="/Users/$TBUSERNAME/Library/Application Support/Halonet/Configurations/$TBSUFFIX"
else
    readonly TBCONFIG="${config}"
fi

readonly CONFIG_PATH_DASHES_SLASHES="$( echo "${TBCONFIG}" | sed -e 's/-/--/g' | sed -e 's/\//-S/g' )"
readonly SCRIPT_LOG_FILE="/Library/Application Support/Halonet/Logs/${CONFIG_PATH_DASHES_SLASHES}.script.log"
...
#
# If Halonet.app is located in /Applications, we load the launchd .plist directly from within the .app.
#
# If Halonet.app is not located in /Applications (i.e., we are debugging), we create a modified version of the launchd .plist and use
...
# in the .plist is changed to point to the copy of the program that is inside the running Halonet.)
#
...
if [ "${ARG_TB_PATH}" = "/Applications/Halonet.app" ] ; then
    ...
    readonly LEASEWATCHER_PLIST_PATH="/Library/Application Support/Halonet/${LEASEWATCHER_PLIST}"
...
if ${ARG_TAP} ; then
	...
	if [ "$bRouteGatewayIsDhcp" == "true" ]; then
		...
		if [ -z "$dev" ]; then
            ...
            # We don't create the "/tmp/halonet-downscript-needs-to-be-run.txt" file, because the down script does NOT need to be run since we didn't do anything
...
touch "/tmp/halonet-downscript-needs-to-be-run.txt"
